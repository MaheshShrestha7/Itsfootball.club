...
export default function EventDoorCheckinPage({
  ...
  // If the visitor is signed in as an approved member of this club, offer a
  // one-tap check-in instead of asking them to also scan/enter their own pass.
  const [signedInMemberName, setSignedInMemberName] = useState<string | null>(null);
  const [signedInMemberToken, setSignedInMemberToken] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    client.auth.getSession().then(async ({ data }) => {
      ...
      const { data: rows } = await client.rpc('claim_member_profile', { p_club_id: club.id });
      if (cancelled) return;
      const row = Array.isArray(rows) ? rows[0] : undefined;
      if (row?.qr_code_token) {
        setSignedInMemberName(row.full_name);
        setSignedInMemberToken(row.qr_code_token);
      }
    });
      ...
      <div style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        <div className="glass-panel text-center" style={{ maxWidth: '480px', padding: '2.5rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '1.5rem' }}>
            The requested event does not exist or has been removed from the schedule.
          </p>
  ...
  const applyCheckinResult = (res: { success: boolean; message: string; attendeeName?: string }) => {
    if (res.success) {
      try {
        confetti({
          particleCount: 90,
        ...
        {!event.door_qr_checkin_enabled ? (
          ...
          >
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', maxWidth: '420px', margin: '0 auto 1.75rem auto' }}>
              Door QR code self-check-in has not been activated by organizers for this event. Please check in with staff at the entrance.
            </p>
            <Link href={`/${club.slug}/events/${event.id}`} className="btn btn-secondary">
              Go to Event Details
            </Link>
          </div>
        ) : result?.success ? (
          ...
          >
            ...
            >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '0.5rem' }}>
                ...
                <span style={{ color: '#FFFFFF', fontWeight: 700 }}>{event.location}</span>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
              ...
              <Link href={`/${club.slug}/events/${event.id}`} className="btn btn-primary btn-sm">
                Event Details
              </Link>
            </div>
          </div>
          ...
          >
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                Scan or enter your credentials to check in for this event.
              </p>
            </div>
            ...
            {/* Signed-in member: one-tap check-in, no need to also scan/enter a pass */}
            {signedInMemberName && signedInMemberToken && (
              <div
                style={{
            ...
            {mode === 'camera' ? (
              <div style={{ marginBottom: '1rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <CameraQRScanner
                  ...
                  scannerId="event-door-camera-station"
                />
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.75rem', textAlign: 'center' }}>
                  Point your camera at your digital membership QR code or printed ticket.
                </span>
              </div>
            ) : (
              <form onSubmit={handleCheckIn}>
                {mode === 'member' ? (
                  <div className="form-group" style={{ marginBottom: '1.5rem' }}>
                    ...
                    <input
                      ...
                      placeholder="Pass token or member ID"
                      value={memberToken}
                    ...
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.35rem', display: 'block' }}>
                      Tip: Enter your pass token from your Digital Member Card to earn ClubScore points!
                    </span>
                  </div>
                ) : (
                  <>
                ...
                >
                  <UserCheck size={18} />
                  <span>{submitting ? 'Checking In...' : 'Complete Check-In'}</span>
                </button>
              </form>
            )}
          </div>
